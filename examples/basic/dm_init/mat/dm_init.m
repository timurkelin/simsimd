...
dm_init_prm = [ ...
    struct( 'var', 'dm_init_data_a', 'len',  100 ); ...
    struct( 'var', 'dm_init_data_b', 'len',  200 ); ...
    struct( 'var', 'dm_init_data_c', 'len',  300 ); ...
    struct( 'var', 'dm_init_data_d', 'len',  400 )];

for init_idx = 1:numel( dm_init_prm )
    
    ram_len = floor( dm_init_prm( init_idx ).len / ram_num );
    ...
    for ram_addr = 1:( ram_len - 1 )
        for ram_idx = 1:ram_num
            if ram_idx == 1
                ...
                ram_data( ram_addr, ram_idx ) = complex( randn( 1 ), randn( 1 ));   
            end
        end
    ...
    if( init_idx == 1 )
        save( 'dm_init.mat', dm_init_prm( init_idx ).var );
    else
        save( 'dm_init.mat', dm_init_prm( init_idx ).var, '-append' );
    end
end
