/*
 * simd_sys_dm_ram_1rw.h
 *
 *  Description:
 *    Declaration of the 4-way 1r 1w ram with a single address bus
 */

#ifndef SIMD_SYS_DM_INCLUDE_SIMD_SYS_DM_RAM_1RW_H_
#define SIMD_SYS_DM_INCLUDE_SIMD_SYS_DM_RAM_1RW_H_
...
namespace simd {
   ...
   class simd_sys_dm_ram_1rw_c
   : public simd::simd_sys_dmeu_c { // declare module class
   public:
      // Constructor
      simd_sys_dm_ram_1rw_c(
      ...
      static inline std::string func( void ) { return "dm_ram_1rw"; }
   ...
   private:
      ...
      std::map<std::string, boost::optional<simd_sys_dm_ag_c &>>  ag_list;       // list of AGs

      // These 2 enablers are mutually exclusive for ram 1rw (only 1 type of transactions is enabled at a time)
      ...
      // Accesses to memory array (more or less represent HW)
      ...
      typedef enum {
         ...
         ACC_MEM_DATA       = 3,
         ACC_MEM_WACK       = 4
      } simd_dm_ram_1rw_acc_t;

   }; // class simd_sys_dm_ram_1rw_c
} // namespace simd

#endif /* SIMD_SYS_DM_INCLUDE_SIMD_SYS_DM_RAM_1RW_H_ */
